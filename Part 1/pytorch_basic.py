...
def multiples_of_ten(start, stop):
  ...
  """
  assert start <= stop
  multiples = [num for num in range(start, stop + 1) if num % 10 == 0]
  return torch.tensor(multiples, dtype=torch.float64) if multiples else torch.empty(0, dtype=torch.float64)
...
def slice_assignment_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x[:2, 0] = 0  # First two rows, first column
  x[:2, 1] = 1  # First two rows, second column
  x[:2, 2:6] = 2  # First two rows, columns 2-5
  x[2:4, 0] = 3  # Rows 2-3, first column
  x[2:4, 1] = 4  # Rows 2-3, second column
  x[2:4, 2:4] = 3  # Rows 2-3, columns 2-3
  x[2:4, 3:4] = 4  # Rows 2-3, fourth column
  x[2:4, 4:6] = 5  # Rows 2-3, columns 4-5
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def zero_row_min(x):
  ...
  y[torch.arange(y.shape[0]), min_indices] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  # Replace "pass" statement with your code
  if use_loop:
    z = torch.stack([x[i] @ y[i] for i in range(x.shape[0])])  # Loop over batch
  else:
    z = torch.bmm(x, y)  # Efficient batched matrix multiplication
  return z
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def normalize_columns(x):
  ...
  M = x.shape[0]
  mean = x.sum(dim=0) / M
  std = torch.sqrt(((x - mean) ** 2).sum(dim=0) / (M-1))
  # print(mean)
  y = (x - mean) / std  # Normalize
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def mm_on_gpu(x, w):
  """
  Perform matrix multiplication on GPU (CUDA) or Apple Silicon GPU (MPS)
